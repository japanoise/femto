...
void add_undo(buffer_t * bp, char type, point_t p, char_t * str, char_t * rep)
{
	...
	if (bp->b_utail != NULL && bp->b_utail->u_type == type && type == UNDO_T_INSERT
	    && (bp->b_utail->u_point + 1) == p) {
	...
	} else if (bp->b_utail != NULL && bp->b_utail->u_type == type && type == UNDO_T_BACKSPACE
		   && (bp->b_utail->u_point - len) == p) {
	...
	} else if (bp->b_utail != NULL && bp->b_utail->u_type == type && type == UNDO_T_DELETE
		   && (bp->b_utail->u_point) == p) {
	...
	} else if (bp->b_utail != NULL && bp->b_utail->u_type == type && type == UNDO_T_INSAT
		   && (bp->b_utail->u_point) == p) {
...
undo_tt *execute_undo(undo_tt * up)
{
	...
	case UNDO_T_REPLACE:
		curbp->b_point = up->u_point;
		replace_string(curbp, (char *)up->u_replace, (char *)up->u_string,
			       strlen((char *)up->u_replace), strlen((char *)up->u_string));
		break;
	}
...
{
	if (up->u_string != NULL)
		return ((int)sizeof(undo_tt) + strlen((char *)up->u_string));
	...
	while (prev != NULL) {
		...
		if (prev->u_type == UNDO_T_REPLACE) {
			...
			sprintf(report_line, "%03d %6s %9ld %4d %s -> %s\n", ++count,
				get_undo_type_name(prev), prev->u_point, size, buf1, buf2);
		} else {
			safe_strncpy((char *)buf1, (char *)str, 40);
			remove_control_chars(buf1);
			sprintf(report_line, "%03d %6s %9ld %4d %s\n", ++count,
				get_undo_type_name(prev), prev->u_point, size, buf1);
		}
